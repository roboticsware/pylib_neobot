...
- Low level OS requests (_keyboard._os_keyboard) are mocked out by rewriting
...
_keyboard,_listener.direct_callback, then, if accepted, appended to
`output_events`. Fake OS events (_keyboard.press) are processed
...
class TestKeyboard(unittest.TestCase):
    def tearDown(self):
        keyboard.unhook_all()
        #self.assertEquals(_keyboard._hooks, {})
        #self.assertEquals(_keyboard._hotkeys, {})
    ...
    #    self.do(d_shift)
    #    _keyboard.write('a', restore_state_after=False, exact=False)
    ...
    #    _keyboard.add_abbreviation('abc', 'aaa')
    #    self.do(du_a+du_b+du_c+du_space, [])
